<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['product:products:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['product:products:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['product:products:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['product:products:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="productsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="参数" align="center" prop="specification" min-width="80">
        <template #default="scope">
          <el-button type="primary" size="small" @click="openEditor(scope.row.specification, scope.row.type)">编辑</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" min-width="80">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['product:products:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['product:products:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
 ...
 <div>
    ...
    >
      <div>
        <h3>Box</h3>
        <el-table :data="Object.entries(localJsonData.box.panels).map(([name, panel]) => ({ name, ...panel }))" style="width: 100%" size="small" :header-cell-style="{ textAlign: 'center' }">
          <el-table-column prop="name" label="position" width="100" />
        ...
        <el-table :data="localJsonData.box.shelves" style="width: 100%" size="small" :header-cell-style="{ textAlign: 'center' }">
        ...
        <el-table :data="localJsonData.face" style="width: 100%" size="small" :header-cell-style="{ textAlign: 'center' }">
          <el-table-column label="type">
            <template #default="scope">
...
const openEditor = (data, type) => {
  console.log(type);
  currentType.value = type;
  localJsonData.value = data ? JSON.parse(JSON.stringify(data)) : {
    box: {
      panels: {
        left: { width: 0, height: 0, depth: 0, material: 'plywood', price: 0 },
        right: { width: 0, height: 0, depth: 0, material: 'plywood', price: 0 },
...
const onSave = () => {
  specification.value = localJsonData.value;
  console.log('保存的 JSON 数据:', specification.value);
